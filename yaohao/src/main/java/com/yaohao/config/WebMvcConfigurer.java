...
import com.yaohao.core.Result;
import com.yaohao.core.ResultCode;
import com.yaohao.core.ServiceException;
import com.yaohao.util.RequestWrapperHelper;
import com.yaohao.util.TimeOutIntercepter;
...
//	}
	...
	// 解决跨域问题
	/**
	 * JSONP跨域，script标签请求资源不受同源策略限制 只能用于get类型 CORS跨域
	 * 
	 * Cors CORS是一个w3c标准,全称是"跨域资源共享"(Cross-origin resource sharing),
	 * 但一个请求url的协议,域名,端口三者之间任意与当前页面地址不同即为跨域.
	 * 它允许阅览器向跨源服务器发送ajax请求,从而克服AJAX只能同源使用的限制. （1) 请求方法是以下三种方法之一：
	 * 
	 ...
	 * 设置允许请求的域名，多个域名以逗号分隔 Access-Control-Allow-Methods: GET, POST, PUT, DELETE,
	 * OPTIONS // 设置允许请求的方法，多个方法以逗号分隔 Access-Control-Allow-Headers:
	 * Authorization // 设置允许请求自定义的请求头字段，多个字段以逗号分隔
	 * Access-Control-Allow-Credentials: true // 设置是否允许发送 Cookies ajax 同源策略
	 * 同域资源可读写; 跨域请求会直接被浏览器拦截. (chrome下跨域请求不会发起, 其他浏览器一般是可发送跨域请求, 但响应被浏览器拦截)
	 */
	@Override
	...
	public void addInterceptors(InterceptorRegistry registry) {
		TimeOutIntercepter weixinSMSIntercepter = new TimeOutIntercepter();
		String allowUrls[] = { "" };
	...
	/**
	 * 验证RequestBody中参数的加密是否正确 验证过程与validateSign(HttpServletRequest request)方法相同
	 *
	 * @param request
	 * @return
	 * @throws IOException
	...
	private boolean validateRequestBodySign(HttpServletRequest request) throws IOException {
		...
		try {
			...
			logger.info(e.toString() + " : " + e.getMessage() + "RequestBody数据验证时，数据出现异常");
			return false;
		}
	...
	public void addResourceHandlers(ResourceHandlerRegistry registry) {
		 ...
		 * "file:D:/Workspaces/Test/productRemoval/download/");
		 */
	}
}
